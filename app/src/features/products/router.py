...
async def get_one_product_by_id(
    ...
    # if user:
    #   background_tasks.add_task(last_seen_products_service._upsert, user.id, product_id)
    background_tasks.add_task(last_seen_products_service._upsert, 1, product_id)
...
async def delete_one_product_by_id(
    ...
    return await product_service.delete_one_by_id(product_id=product_id)
...
async def soft_delete_one_product_by_id(
    ...
    await product_service.soft_delete_one_by_id(product_id=product_id)
...
async def update_one_product_by_id(
    ...
    await product_service.update_one_by_id(
